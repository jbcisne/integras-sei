...
class IncluirProcessoBloco extends IntegrasSEI300 implements IntegrasServiceInterface, ExtractInterface
{
    ...
    private function loadParameters(array $args)
    {
        ...
        if (!isset($args[3])) {
            IntegrasServicesArgumentException::argumentRequired('anotacao');
        }

        $this->idUnidade = isset($args[0]) ? $args[0] : null;
        $this->idBloco = isset($args[1]) ? $args[1] : null;
        $this->protocoloProcedimento = isset($args[2]) ? $args[2] : null;
        $this->anotacao = isset($args[3]) ? $args[3] : null;
    }
