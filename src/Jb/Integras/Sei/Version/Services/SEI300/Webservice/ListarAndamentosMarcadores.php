...
class ListarAndamentosMarcadores extends IntegrasSEI300 implements IntegrasServiceInterface, ExtractInterface
{
    ...
    private function loadParameters(array $args)
    {
        ...
        if (!isset($args[2])) {
            IntegrasServicesArgumentException::argumentRequired('marcadores');
        }

        $this->idUnidade = isset($args[0]) ? $args[0] : null;
        $this->protocoloProcedimento = isset($args[1]) ? $args[1] : null;
        $this->marcadores = isset($args[2]) ? $args[2] : null;
    }
